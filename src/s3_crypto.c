/*
 * libs3 — Cryptographic primitives
 *
 * SHA-256 (FIPS 180-4), HMAC-SHA256 (RFC 2104), SHA-1 (FIPS 180-4),
 * CRC32, CRC32C, Base64, URI encoding, Hex encoding.
 *
 * Pure C23, no external crypto dependency.
 */
...
void s3__hmac_sha256(const void *key, size_t key_len,
    ...
    s3_sha256_ctx ctx;
    s3__sha256_init(&ctx);
    s3__sha256_update(&ctx, k_pad, 64);
    s3__sha256_update(&ctx, data, data_len);
    uint8_t inner[32];
    s3__sha256_final(&ctx, inner);
    ...
    s3__sha256_init(&ctx);
    s3__sha256_update(&ctx, k_pad, 64);
    s3__sha256_update(&ctx, inner, 32);
    s3__sha256_final(&ctx, out);
...
static inline uint32_t sha1_rotl(uint32_t x, unsigned n) {
    return (x << n) | (x >> (32 - n));
}
...
size_t s3__base64_decode(const char *in, size_t in_len, uint8_t *out, size_t out_size) {
    ...
    while (in_len > 0 && (in[in_len - 1] == '=' || in[in_len - 1] == '\n' ||
                          in[in_len - 1] == '\r' || in[in_len - 1] == ' ')) {
        in_len--;
    }

    /* Calculate output length:
     * Every 4 base64 chars produce 3 bytes.
     * Remaining 2 chars produce 1 byte, remaining 3 chars produce 2 bytes. */
    ...
    else if (remainder == 1) return 0; /* Invalid */
    ...
    for (; i + 3 < in_len; i += 4) {
        ...
        int c = b64_decode_char(in[i + 2]);
        int d = b64_decode_char(in[i + 3]);
    ...
    if (remainder == 2) {
        ...
        out[j++] = (uint8_t)(((uint32_t)a << 2) | ((uint32_t)b >> 4));
        out[j++] = (uint8_t)((((uint32_t)b & 0xF) << 4) | ((uint32_t)c >> 2));
    }
...
/* ═══════════════════════════════════════════════════════════════════════════
 * URI Encoding (AWS-specific percent encoding)
 * ═══════════════════════════════════════════════════════════════════════════ */

static const char hex_upper[] = "0123456789ABCDEF";

static inline bool uri_unreserved(unsigned char c) {
    return (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') ||
           (c >= '0' && c <= '9') || c == '-' || c == '.' ||
           c == '_' || c == '~';
}

size_t s3__uri_encode(const char *in, size_t in_len, char *out, size_t out_size,
                      bool encode_slash) {
    size_t j = 0;
    for (size_t i = 0; i < in_len; i++) {
        unsigned char c = (unsigned char)in[i];
        if (uri_unreserved(c) || (c == '/' && !encode_slash)) {
            if (j + 1 >= out_size) return 0;
            out[j++] = (char)c;
        } else {
            if (j + 3 >= out_size) return 0;
            out[j++] = '%';
            out[j++] = hex_upper[(c >> 4) & 0x0F];
...
size_t s3__uri_encode_path(const char *in, size_t in_len, char *out, size_t out_size) {
    return s3__uri_encode(in, in_len, out, out_size, false);
}
...
int s3__hex_decode(const char *in, size_t in_len, uint8_t *out, size_t out_size) {
    ...
    if (out_size < decoded_len) return -1;
...
/* ═══════════════════════════════════════════════════════════════════════════
 * MD5 (RFC 1321) — minimal implementation for Content-MD5 header
 * ═══════════════════════════════════════════════════════════════════════════ */
...
void s3__md5(const void *data, size_t len, uint8_t hash[16]) {
    ...
    memcpy(buf, p + i, remaining);
    buf[remaining++] = 0x80;

    /* Need room for 8-byte length at end of a 64-byte block */
    size_t pad_to = (remaining <= 56) ? 56 : 120;
    memset(buf + remaining, 0, pad_to - remaining);

    /* Append length in bits (little-endian) */
    uint64_t bit_len = (uint64_t)len * 8;
    for (int j = 0; j < 8; j++)
        buf[pad_to + j] = (uint8_t)(bit_len >> (j * 8));

    md5_transform(state, buf);
    if (pad_to > 56)
        md5_transform(state, buf + 64);

    /* Output (little-endian) */
    for (int j = 0; j < 4; j++) {
        hash[j*4+0] = (uint8_t)(state[j]);
